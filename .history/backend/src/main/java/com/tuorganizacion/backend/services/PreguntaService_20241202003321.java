...
public class PreguntaService {
    ...
    ObjectMapper mapper = new ObjectMapper();
     // Esto convierte el JSON en una lista de mapas, donde cada mapa representa una pregunta
    List<Map<String, Object>> preguntasList = new ArrayList<>();
        
    // Dividir las preguntas para deserializarlas correctamente si es necesario
    String[] preguntasSeparadas = preguntasJson.split("(?<=\\})\\s*(?=\\{)");  // Divide el JSON en cada pregunta individual
    for (String preguntaJson : preguntasSeparadas) {
        Map<String, Object> preguntaMap = mapper.readValue(preguntaJson, new TypeReference<Map<String, Object>>() {});
        preguntasList.add(preguntaMap);
    }
